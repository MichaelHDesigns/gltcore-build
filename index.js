 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`gltcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`gltcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'gltcore-' + name : 'gltcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/gltcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external gltcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:gltcore-lib -o gltcore-lib.js';
    }
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push globaltoken $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
